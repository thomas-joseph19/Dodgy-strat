...
phase: 15
plan_id: "03"
title: "Trade Stream Normaliser and Equity Curve Builder"
wave: 2
depends_on: ["01", "02"]
autonomous: true
files_modified:
...
# Plan 03: Trade Stream Normaliser and Equity Curve Builder

## Objective
Create a Python module that reads both the Daniel trades CSV and LG Model trades CSV, normalises them into a common schema, merges them chronologically, and builds equity curves for each strategy and their combined performance.

## Tasks

<task id="1" title="Create src/trade_merger.py">
<read_first>
- src/metrics.py — `get_performance_summary()` to understand what format it expects
- strategies/lg_model/src/reporting/trade-csv.js — LG's CSV column schema: `session_date, side, entry_time_utc, exit_time_utc, entry_price, initial_stop, final_stop, tp1, tp2, exit_category, pnl_points, pnl_usd, max_favorable_excursion_points, time_to_be_bars`
- main.py — Daniel's `trade_rows` dict keys: `date, setup_id, direction, session, entry_price, stop_price, target_price, risk_points, exit_price, exit_type, pnl_points, pnl_usd, r_multiple, risk_reward, reasoning`
</read_first>

<action>
Create `src/trade_merger.py` with the following implementation:
...
<acceptance_criteria>
- `src/trade_merger.py` exists
- File contains `def normalise_daniel_trades(`
- File contains `def normalise_lg_trades(`
- File contains `def merge_and_sort(`
- File contains `def build_equity_curves(`
- File contains `def compute_combined_summary(`
- File contains `LG_DOLLARS_PER_POINT = 20.0`
- File contains logic to exclude `EXPIRED_OR_STILL_OPEN` from Daniel trades
- File contains `CLOSED_EXITS` set for LG trade filtering
- Module imports without error: `python -c "from src.trade_merger import merge_and_sort; print('OK')"`
</acceptance_criteria>
</task>

## Verification
Run: `python -c "from src.trade_merger import normalise_daniel_trades, normalise_lg_trades, build_equity_curves; print('Module OK')"`

## must_haves
- Both normalise functions handle missing/empty fields gracefully
- Equity curve formula uses shared $100k base (not $200k double-counting)
- LG trades are filtered to only CLOSED exit categories
- Daniel EXPIRED_OR_STILL_OPEN trades are excluded
- Chronological merge sorts by `exit_time_sort` UTC string
